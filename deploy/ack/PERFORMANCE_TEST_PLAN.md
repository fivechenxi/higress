...
3. request rate divided by the tested safe RPS-per-Pod.
...
stream signal. Start with a provisional target of 100 active requests per Pod
and re-calibrate it with the production plugin chain and a non-saturated mock.
The current raw-Envoy test held 250 streams per Pod without errors, but the
mock/load generator reached its own latency knee, so 250 is not yet a valid
...
The exact RPS/stream thresholds must come from the first saturation knee, not a
generic vendor number. Define safe capacity as 70% of the highest load at which
all latency/error SLOs pass, and keep at least 30% headroom for failover of one
gateway Pod.
...
Applying the plan's 70% rule gives a provisional CPU-path target of about 125
RPS per Pod. This is deliberately conservative and must be replaced after
three repeated runs with the production plugins enabled.
...
the active-request custom metric before production traffic.
...
- Before production, expose Envoy metrics through ACK Managed Service for
  Prometheus/metrics adapter and add an active-request target, initially 100 per
  Pod, plus a provisional request-rate target of 125 RPS per Pod.
