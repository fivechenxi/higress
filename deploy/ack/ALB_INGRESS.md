...
1. Set `ack_alb_ingress_controller_enabled=true`. OpenTofu installs the ACK-managed `alb-ingress-controller` addon with `CreateDefaultALBConfig=false` and waits for the `alibabacloud.com/v1` AlbConfig CRD to become established before Helm runs. ACK Basic and Pro do not expose a controller Deployment in `kube-system`, so do not use a Deployment rollout as the readiness check. Confirm the chosen AlbConfig and IngressClass names are unused. The default new name is `higress-gateway-alb`; the chart intentionally omits `spec.config.id` so the controller creates a new ALB. The managed addon does not create an unrelated default ALB.
...
5. Review an isolated Helm diff and the current CLB/OpenTofu plan before any deployment. The chart remains disabled by default. Production activation is stored in the OSS-managed `terraform.tfvars`, using `higress_alb_ingress_enabled`, `higress_alb_vswitch_ids`, `higress_alb_certificate_id`, `higress_alb_request_timeout`, and `higress_alb_idle_timeout`. DNS cutover is separately persisted with `higress_alb_dns_enabled` and `higress_alb_dns_name`. These are desired settings, not proof of live cloud state. Do not replace the Gateway Service type or remove its CLB annotations.
...
Enabling the OSS-managed ALB inputs authorizes the controller to create the separate pay-as-you-go ALB, EIPs, listeners, server group and forwarding rule. DNS remains on the CLB until the separately reviewed `higress_alb_dns_enabled=true` and exact controller-reported `higress_alb_dns_name` are applied.
...
4. Plan the `api` DNS switch **separately** from Helm. Set `higress_alb_dns_enabled=true` and `higress_alb_dns_name` to the exact controller-reported DNS name. OpenTofu then keeps the existing managed record ID but declares it as the ALB CNAME, so later applies cannot silently restore the CLB A record. Preserve the 600-second cache window and keep both edges healthy until propagation is observed.
