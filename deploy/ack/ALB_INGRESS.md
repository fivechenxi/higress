...
1. Set `ack_alb_ingress_controller_enabled=true`. OpenTofu installs the ACK-managed `alb-ingress-controller` addon and waits for its Deployment and the `alibabacloud.com/v1` AlbConfig CRD before Helm runs. Confirm the chosen AlbConfig and IngressClass names are unused. The default new name is `higress-gateway-alb`; the chart intentionally omits `spec.config.id` so the controller creates a new ALB. The managed addon is installed without a default AlbConfig, so it does not create an unrelated default ALB.
